  ...
  if ($numRegistros > 0){
   ...
   if($linhaAcessadaCadastro || $linhaAcessadaReatOrAlt){
     ...
     if( $arrObjSubtemaRIDTO[$i]->getStrSinAtivo()=='S' ){
      ...
      $strResultado .= '<td>'. PaginaSEI::tratarHTML( $arrObjSubtemaRIDTO[$i]->getStrClassificacaoTema() ); '</td>';
      $strResultado .= '<td>'. PaginaSEI::tratarHTML( $arrObjSubtemaRIDTO[$i]->getStrNomeSubtema() ); '</td>';
      ...
      if ( $bolAcaoDesativar && $arrObjSubtemaRIDTO[$i]->getStrSinAtivo() == 'S'){
        $strResultado .= '<a href="'.PaginaSEI::getInstance()->montarAncora($strId).'" onclick="acaoDesativar(\''.$strId.'\',\''.$strDescricao.'\',\''.$idSubtema.'\');" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/desativar.gif" title="Desativar Tema" alt="Desativar Tema" class="infraImg" /></a>&nbsp;';
      }

      if( $bolAcaoReativar && $arrObjSubtemaRIDTO[$i]->getStrSinAtivo() == 'N' ) {
	    $strResultado .= '<a href="'.PaginaSEI::getInstance()->montarAncora($strId).'" onclick="acaoReativar(\''.$strId.'\',\''.$strDescricao.'\',\''.$idSubtema.'\');" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/reativar.gif" title="Reativar Tema" alt="Reativar Tema" class="infraImg" /></a>&nbsp;';
      }

      if ($bolAcaoExcluir){
        $strResultado .= '<a href="'.PaginaSEI::getInstance()->montarAncora($strId).'" onclick="acaoExcluir(\''.$strId.'\',\''.$strDescricao.'\',\''.$idSubtema.'\');" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/excluir.gif" title="Excluir Tema" alt="Excluir Tema" class="infraImg" /></a>&nbsp;';
      }
...
<style type="text/css">
    ...
    #frmClassificacaoTemaRILista label[for^=txt] {display: block;}
    #frmClassificacaoTemaRILista label[for^=sel] {display: block;}
    #frmClassificacaoTemaRILista .bloco {position: relative;float: left; margin-bottom: 1%;}
    #frmClassificacaoTemaRILista .selClass {display: inline !important;border: .1em solid #666; width: 200px;}
...
<form id="frmClassificacaoTemaRILista" method="post" action="<?=PaginaSEI::getInstance()->formatarXHTML(SessaoSEI::getInstance()->assinarLink('controlador.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao']))?>">

  <?
  PaginaSEI::getInstance()->montarBarraComandosSuperior($arrComandos);
  ?>
    <?php PaginaSEI::getInstance()->abrirAreaDados(); ?>
        <div class="bloco" style="width: auto; margin-right: 5%">
            <label id="lblTema" for="selTema" class="infraLabelOpcional" tabindex="445">
                Tema:
            </label>

            <select tabindex="446" onchange="filtrarClassificacaoTema();" id="selTema" name="selTema" class="infraSelect selClass">
        ...
        <div class="bloco" style="width: auto">
            ...
            <select tabindex="448" onchange="filtrarClassificacaoTema();" id="selSubtema" name="selSubtema" class="infraSelect selClass">
                <option value="" selected="selected">
                    Todos
                </option>
