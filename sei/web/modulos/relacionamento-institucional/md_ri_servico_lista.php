<?php
    ...
    if ($numRegistros > 0) {
        ...
        for ($i = 0; $i < $numRegistros; $i++) {
            ...
            #Ação Consulta
            if (!$bolSelecionar) {
                $strResultado .= '<a href="' . PaginaSEI::getInstance()->formatarXHTML(SessaoSEI::getInstance()->assinarLink($strUrl . 'consultar&acao_origem=' . $_GET['acao'] . '&acao_retorno=' . $_GET['acao'] . '&id_servico_relacionamento_institucional=' . $strId)) . '" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/consultar.gif" title="Consultar Serviço" alt="Consultar Serviço" class="infraImg" /></a>&nbsp;';

                #Ação Alterar
                 $strResultado .= '<a href="' . PaginaSEI::getInstance()->formatarXHTML(SessaoSEI::getInstance()->assinarLink($strUrl . 'alterar&acao_origem=' . $_GET['acao'] . '&acao_retorno=' . $_GET['acao'] . '&id_servico_relacionamento_institucional=' . $strId)) . '" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/alterar.gif" title="Alterar Serviço" alt="Alterar Serviço" class="infraImg" /></a>&nbsp;';

                #Ação Desativar
                if ($bolRegistroAtivo) {
                    $strResultado .= '<a href="' . PaginaSEI::getInstance()->montarAncora($strId) . '" onclick="desativar(\'' . $strId . '\',\'' . PaginaSEI::getInstance()->formatarParametrosJavaScript($strNomeServico) . '\');" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/desativar.gif" title="Desativar Serviço" alt="Desativar Serviço" class="infraImg" /></a>&nbsp;';
                }

                #Ação Reativar
                if (!$bolRegistroAtivo) {
                    $strResultado .= '<a href="' . PaginaSEI::getInstance()->montarAncora($strId) . '" onclick="reativar(\'' . $strId . '\',\'' . PaginaSEI::getInstance()->formatarParametrosJavaScript($strNomeServico) . '\');" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/reativar.gif" title="Reativar Serviço" alt="Reativar Serviço" class="infraImg" /></a>&nbsp;';
                }

                #Ação Excluir
                $strResultado .= '<a href="' . PaginaSEI::getInstance()->montarAncora($strId) . '" onclick="excluir(\'' . $strId . '\',\'' . PaginaSEI::getInstance()->formatarParametrosJavaScript($strNomeServico) . '\');" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/excluir.gif" title="Excluir Serviço" alt="Excluir Serviço" class="infraImg" /></a>&nbsp;';
            } else {
...
?>
    <form id="frmServicoLista" method="post"
        ...
        <div id="divInfraAreaDados" class="infraAreaDados" style="height:4.5em;">
            ...
            <div>
                <input type="text" id="txtServico" name="txtServico" class="infraText" size="30"
                       value="<?= isset($_POST['txtServico']) ? $_POST['txtServico'] : '' ?>" maxlength="100"
                       tabindex="502"/>
            </div>
        </div>
