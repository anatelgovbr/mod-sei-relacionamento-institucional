...
try {
  ...
  if ($numRegistros > 0){
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      $strResultado .= '<td>'. PaginaSEI::tratarHTML( $arrObjSubtemaRIDTO[$i]->getStrSubtema() ); '</td>';
     ...
     $strResultado .= PaginaSEI::getInstance()->getAcaoTransportarItem($i, $arrObjSubtemaRIDTO[$i]->getNumIdSubtemaRelacionamentoInstitucional());
      ...
      if ( $bolAcaoDesativar && $arrObjSubtemaRIDTO[$i]->getStrSinAtivo() == 'S'){
        $strResultado .= '<a href="'.PaginaSEI::getInstance()->montarAncora($strId).'" onclick="acaoDesativar(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/desativar.gif" title="Desativar Subtema" alt="Desativar Subtema" class="infraImg" /></a>&nbsp;';
      } 
      
      if( $bolAcaoReativar && $arrObjSubtemaRIDTO[$i]->getStrSinAtivo() == 'N' ) {
	    $strResultado .= '<a href="'.PaginaSEI::getInstance()->montarAncora($strId).'" onclick="acaoReativar(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/reativar.gif" title="Reativar Subtema" alt="Reativar Subtema" class="infraImg" /></a>&nbsp;';
      }
      if ($bolAcaoExcluir){
        $strResultado .= '<a href="'.PaginaSEI::getInstance()->montarAncora($strId).'" onclick="acaoExcluir(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/excluir.gif" title="Excluir Subtema" alt="Excluir Subtema" class="infraImg" /></a>&nbsp;';
      }
...
function acaoDesativar(id,desc){
  ...
  if (confirm("Confirma desativação do Subtema \""+desc+"\"?")){
...
function acaoReativar(id,desc){
  if (confirm("Confirma reativação do Subtema \""+desc+"\"?")){
    document.getElementById('hdnInfraItemId').value=id;
...
function acaoExcluir(id,desc){
  ...
  if (confirm("Confirma exclusão do Subtema \""+desc+"\"?")){
    document.getElementById('hdnInfraItemId').value=id;
...
#lblSubtema {position:absolute;left:0%;top:0%;width:30%;}
#txtSubtema {position:absolute;left:0%;top:40%;}
