...
class MdRiAtualizadorSeiRN extends InfraRN {
  ...
  protected function instalarv100(){
	...
	$this->logar(' EXECUTANDO A INSTALACAO DA VERSAO'.$this->versaoAtualDesteModulo.  'DO MODULO '.$this->nomeDesteModulo.', NA BASE DO SEI ');
	 ...
	 //inserindo registro inicial padrao, setando a data de corte para a data da execuçao deste script
	...
	//rel demanda externa x municipios
	...
	//rel demanda externa x serviços
	...
	//rel demanda externa x classificaçao por temas
	//CRIANDO CLASSIFICACAO POR TEMA
	...
	//Criando a REL de CLASS TEMA X SUBTEMA
	...
	//CRIANDO A REL DE DEMANDA EXTERNA X CLASSIFICACAO POR TEMA
	...
	//rel demanda externa x unidades responsaveis
	...
	//rel demanda externa x entidades
	...
	//rel demanda externa x tipos de controle
	//CRIANDO TIPO CONTROLE DEMANDA
	...
	//CRTANDO TABELAS REL DE REITERACAO
	...
	$objInfraMetaBD->adicionarChaveEstrangeira('fk2_md_ri_rel_cad_localidade','md_ri_rel_cad_localidade',
  	...
  	//============================== atualizando parametro para controlar versao do modulo
		...
		BancoSEI::getInstance()->executarSql("INSERT INTO infra_parametro (nome, valor) values('" . $this->nomeParametroModulo . "','" . $this->versaoAtualDesteModulo . "')");
		BancoSEI::getInstance()->confirmarTransacao();

  	} catch( Exception $e){

  		$this->logar( $e->getTraceAsString() . " ::: " . $e->getMessage() );
